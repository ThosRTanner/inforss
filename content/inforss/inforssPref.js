   ...
   try
   {
     ...
     switch (rss.getAttribute("type"))
     {
       ...
       case "nntp":
       {
         ...
         document.getElementById('prefDescription').value = rss.getAttribute("description");
    	...
    	if (originalFeed != null)
    	{
    	  ...
    	  if (originalFeed != null)
    	  {
    	    ...
    	    document.getElementById("inforss.feed.treecell3").setAttribute("label", ((originalFeed.lastRefresh == null)? "" : getStringDate(originalFeed.lastRefresh)));
    	    document.getElementById("inforss.feed.treecell4").setAttribute("label", (((originalFeed.lastRefresh == null) || (originalFeed.active == false) || (rss.getAttribute("activity") == "false"))? "" : getStringDate(new Date(eval(originalFeed.lastRefresh.getTime() + originalFeed.feedXML.getAttribute("refresh") * 60000)))));
     ...
     but.label=document.getElementById("bundle_inforss").getString("inforss.apply");
     but.setAttribute("accesskey","");
     but.addEventListener("click", function () { return apply() }, false);
...
function savePref()
{
  ...
  try
  {
    ...
    {
      returnValue = false;
      var acceptButton = document.getElementById('inforssPref').getButton("accept");
    ...
    if (validDialog() == true)
    {
      ...
      switch (rss.getAttribute("type"))
      {
        ...
        case "nntp":
        {
          ...
          rss.setAttribute("description", document.getElementById('prefDescription').value);
          ...
          itemMenu.setAttribute("label", document.getElementById("prefTitle").value);
        ...
        case "group":
        {
          ...
          itemMenu.setAttribute("label", document.getElementById("groupName").value);
...
//dump("remove filter\n");
      ...
      {
        window.opener.document.getElementById('inforss-icon').setAttribute("tooltiptext",rss.getAttribute("description") + " (" + rss.getAttribute("url") + ")");
      }
  ...
  try
  {
    ...
    switch (rss.getAttribute("type"))
    {
      ...
      case "nntp":
      {
        ...
        if ((rss.getAttribute("type") == "html") && (returnValue == true))
        {
          if ((rss.getAttribute("regexp") == null) || (rss.getAttribute("regexp") == ""))
          {
            returnValue = false;
          ...
          {
            if ((rss.getAttribute("htmlTest") == null) || (rss.getAttribute("htmlTest") == "") || (rss.getAttribute("htmlTest") == "false"))
            {
              returnValue = false;
...
function parseHtml()
{
  try
  {
    var currentRSS = window.opener.inforssGetItemFromUrl(itemMenu.getAttribute("url"));
